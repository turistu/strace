/* Generated by ./src/xlat/gen.sh from ./src/xlat/kexec_file_load_flags.in; do not edit. */

#include "gcc_compat.h"
#include "static_assert.h"

#undef XLAT_PREV_VAL

#ifndef XLAT_MACROS_ONLY

# ifdef IN_MPERS

#  error static const struct xlat kexec_file_load_flags in mpers mode

# else

DIAG_PUSH_IGNORE_TAUTOLOGICAL_CONSTANT_COMPARE
static const struct xlat_data kexec_file_load_flags_xdata[] = {
 XLAT(KEXEC_FILE_UNLOAD),
 #define XLAT_VAL_0 ((unsigned) (KEXEC_FILE_UNLOAD))
 #define XLAT_STR_0 STRINGIFY(KEXEC_FILE_UNLOAD)
 XLAT(KEXEC_FILE_ON_CRASH),
 #define XLAT_VAL_1 ((unsigned) (KEXEC_FILE_ON_CRASH))
 #define XLAT_STR_1 STRINGIFY(KEXEC_FILE_ON_CRASH)
 XLAT(KEXEC_FILE_NO_INITRAMFS),
 #define XLAT_VAL_2 ((unsigned) (KEXEC_FILE_NO_INITRAMFS))
 #define XLAT_STR_2 STRINGIFY(KEXEC_FILE_NO_INITRAMFS)
 XLAT(KEXEC_FILE_DEBUG),
 #define XLAT_VAL_3 ((unsigned) (KEXEC_FILE_DEBUG))
 #define XLAT_STR_3 STRINGIFY(KEXEC_FILE_DEBUG)
 XLAT(KEXEC_FILE_NO_CMA),
 #define XLAT_VAL_4 ((unsigned) (KEXEC_FILE_NO_CMA))
 #define XLAT_STR_4 STRINGIFY(KEXEC_FILE_NO_CMA)
 XLAT(KEXEC_FILE_FORCE_DTB),
 #define XLAT_VAL_5 ((unsigned) (KEXEC_FILE_FORCE_DTB))
 #define XLAT_STR_5 STRINGIFY(KEXEC_FILE_FORCE_DTB)
};
static
const struct xlat kexec_file_load_flags[1] = { {
 .data = kexec_file_load_flags_xdata,
 .size = ARRAY_SIZE(kexec_file_load_flags_xdata),
 .type = XT_NORMAL,
 .flags_mask = 0
#  ifdef XLAT_VAL_0
  | XLAT_VAL_0
#  endif
#  ifdef XLAT_VAL_1
  | XLAT_VAL_1
#  endif
#  ifdef XLAT_VAL_2
  | XLAT_VAL_2
#  endif
#  ifdef XLAT_VAL_3
  | XLAT_VAL_3
#  endif
#  ifdef XLAT_VAL_4
  | XLAT_VAL_4
#  endif
#  ifdef XLAT_VAL_5
  | XLAT_VAL_5
#  endif
  ,
 .flags_strsz = 0
#  ifdef XLAT_STR_0
  + sizeof(XLAT_STR_0)
#  endif
#  ifdef XLAT_STR_1
  + sizeof(XLAT_STR_1)
#  endif
#  ifdef XLAT_STR_2
  + sizeof(XLAT_STR_2)
#  endif
#  ifdef XLAT_STR_3
  + sizeof(XLAT_STR_3)
#  endif
#  ifdef XLAT_STR_4
  + sizeof(XLAT_STR_4)
#  endif
#  ifdef XLAT_STR_5
  + sizeof(XLAT_STR_5)
#  endif
  ,
} };
DIAG_POP_IGNORE_TAUTOLOGICAL_CONSTANT_COMPARE

#  undef XLAT_STR_0
#  undef XLAT_VAL_0
#  undef XLAT_STR_1
#  undef XLAT_VAL_1
#  undef XLAT_STR_2
#  undef XLAT_VAL_2
#  undef XLAT_STR_3
#  undef XLAT_VAL_3
#  undef XLAT_STR_4
#  undef XLAT_VAL_4
#  undef XLAT_STR_5
#  undef XLAT_VAL_5
# endif /* !IN_MPERS */

#endif /* !XLAT_MACROS_ONLY */
