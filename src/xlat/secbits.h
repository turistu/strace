/* Generated by ./src/xlat/gen.sh from ./src/xlat/secbits.in; do not edit. */

#include "gcc_compat.h"
#include "static_assert.h"

#undef XLAT_PREV_VAL

#ifndef XLAT_MACROS_ONLY

# ifdef IN_MPERS

#  error static const struct xlat secbits in mpers mode

# else

DIAG_PUSH_IGNORE_TAUTOLOGICAL_CONSTANT_COMPARE
static const struct xlat_data secbits_xdata[] = {
 XLAT(SECBIT_NOROOT),
 #define XLAT_VAL_0 ((unsigned) (SECBIT_NOROOT))
 #define XLAT_STR_0 STRINGIFY(SECBIT_NOROOT)
 XLAT(SECBIT_NOROOT_LOCKED),
 #define XLAT_VAL_1 ((unsigned) (SECBIT_NOROOT_LOCKED))
 #define XLAT_STR_1 STRINGIFY(SECBIT_NOROOT_LOCKED)
 XLAT(SECBIT_NO_SETUID_FIXUP),
 #define XLAT_VAL_2 ((unsigned) (SECBIT_NO_SETUID_FIXUP))
 #define XLAT_STR_2 STRINGIFY(SECBIT_NO_SETUID_FIXUP)
 XLAT(SECBIT_NO_SETUID_FIXUP_LOCKED),
 #define XLAT_VAL_3 ((unsigned) (SECBIT_NO_SETUID_FIXUP_LOCKED))
 #define XLAT_STR_3 STRINGIFY(SECBIT_NO_SETUID_FIXUP_LOCKED)
 XLAT(SECBIT_KEEP_CAPS),
 #define XLAT_VAL_4 ((unsigned) (SECBIT_KEEP_CAPS))
 #define XLAT_STR_4 STRINGIFY(SECBIT_KEEP_CAPS)
 XLAT(SECBIT_KEEP_CAPS_LOCKED),
 #define XLAT_VAL_5 ((unsigned) (SECBIT_KEEP_CAPS_LOCKED))
 #define XLAT_STR_5 STRINGIFY(SECBIT_KEEP_CAPS_LOCKED)
 XLAT(SECBIT_NO_CAP_AMBIENT_RAISE),
 #define XLAT_VAL_6 ((unsigned) (SECBIT_NO_CAP_AMBIENT_RAISE))
 #define XLAT_STR_6 STRINGIFY(SECBIT_NO_CAP_AMBIENT_RAISE)
 XLAT(SECBIT_NO_CAP_AMBIENT_RAISE_LOCKED),
 #define XLAT_VAL_7 ((unsigned) (SECBIT_NO_CAP_AMBIENT_RAISE_LOCKED))
 #define XLAT_STR_7 STRINGIFY(SECBIT_NO_CAP_AMBIENT_RAISE_LOCKED)
 XLAT(SECBIT_EXEC_RESTRICT_FILE),
 #define XLAT_VAL_8 ((unsigned) (SECBIT_EXEC_RESTRICT_FILE))
 #define XLAT_STR_8 STRINGIFY(SECBIT_EXEC_RESTRICT_FILE)
 XLAT(SECBIT_EXEC_RESTRICT_FILE_LOCKED),
 #define XLAT_VAL_9 ((unsigned) (SECBIT_EXEC_RESTRICT_FILE_LOCKED))
 #define XLAT_STR_9 STRINGIFY(SECBIT_EXEC_RESTRICT_FILE_LOCKED)
 XLAT(SECBIT_EXEC_DENY_INTERACTIVE),
 #define XLAT_VAL_10 ((unsigned) (SECBIT_EXEC_DENY_INTERACTIVE))
 #define XLAT_STR_10 STRINGIFY(SECBIT_EXEC_DENY_INTERACTIVE)
 XLAT(SECBIT_EXEC_DENY_INTERACTIVE_LOCKED),
 #define XLAT_VAL_11 ((unsigned) (SECBIT_EXEC_DENY_INTERACTIVE_LOCKED))
 #define XLAT_STR_11 STRINGIFY(SECBIT_EXEC_DENY_INTERACTIVE_LOCKED)
};
static
const struct xlat secbits[1] = { {
 .data = secbits_xdata,
 .size = ARRAY_SIZE(secbits_xdata),
 .type = XT_NORMAL,
 .flags_mask = 0
#  ifdef XLAT_VAL_0
  | XLAT_VAL_0
#  endif
#  ifdef XLAT_VAL_1
  | XLAT_VAL_1
#  endif
#  ifdef XLAT_VAL_2
  | XLAT_VAL_2
#  endif
#  ifdef XLAT_VAL_3
  | XLAT_VAL_3
#  endif
#  ifdef XLAT_VAL_4
  | XLAT_VAL_4
#  endif
#  ifdef XLAT_VAL_5
  | XLAT_VAL_5
#  endif
#  ifdef XLAT_VAL_6
  | XLAT_VAL_6
#  endif
#  ifdef XLAT_VAL_7
  | XLAT_VAL_7
#  endif
#  ifdef XLAT_VAL_8
  | XLAT_VAL_8
#  endif
#  ifdef XLAT_VAL_9
  | XLAT_VAL_9
#  endif
#  ifdef XLAT_VAL_10
  | XLAT_VAL_10
#  endif
#  ifdef XLAT_VAL_11
  | XLAT_VAL_11
#  endif
  ,
 .flags_strsz = 0
#  ifdef XLAT_STR_0
  + sizeof(XLAT_STR_0)
#  endif
#  ifdef XLAT_STR_1
  + sizeof(XLAT_STR_1)
#  endif
#  ifdef XLAT_STR_2
  + sizeof(XLAT_STR_2)
#  endif
#  ifdef XLAT_STR_3
  + sizeof(XLAT_STR_3)
#  endif
#  ifdef XLAT_STR_4
  + sizeof(XLAT_STR_4)
#  endif
#  ifdef XLAT_STR_5
  + sizeof(XLAT_STR_5)
#  endif
#  ifdef XLAT_STR_6
  + sizeof(XLAT_STR_6)
#  endif
#  ifdef XLAT_STR_7
  + sizeof(XLAT_STR_7)
#  endif
#  ifdef XLAT_STR_8
  + sizeof(XLAT_STR_8)
#  endif
#  ifdef XLAT_STR_9
  + sizeof(XLAT_STR_9)
#  endif
#  ifdef XLAT_STR_10
  + sizeof(XLAT_STR_10)
#  endif
#  ifdef XLAT_STR_11
  + sizeof(XLAT_STR_11)
#  endif
  ,
} };
DIAG_POP_IGNORE_TAUTOLOGICAL_CONSTANT_COMPARE

#  undef XLAT_STR_0
#  undef XLAT_VAL_0
#  undef XLAT_STR_1
#  undef XLAT_VAL_1
#  undef XLAT_STR_2
#  undef XLAT_VAL_2
#  undef XLAT_STR_3
#  undef XLAT_VAL_3
#  undef XLAT_STR_4
#  undef XLAT_VAL_4
#  undef XLAT_STR_5
#  undef XLAT_VAL_5
#  undef XLAT_STR_6
#  undef XLAT_VAL_6
#  undef XLAT_STR_7
#  undef XLAT_VAL_7
#  undef XLAT_STR_8
#  undef XLAT_VAL_8
#  undef XLAT_STR_9
#  undef XLAT_VAL_9
#  undef XLAT_STR_10
#  undef XLAT_VAL_10
#  undef XLAT_STR_11
#  undef XLAT_VAL_11
# endif /* !IN_MPERS */

#endif /* !XLAT_MACROS_ONLY */
